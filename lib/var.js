...
module.exports = function (dust) {
    ...
    dust.helpers['var'] = function (chunk, context, bodies, params) {
        ...
        var name = dust.helpers.tap(params.name, chunk, context),
            ...
            hook = dust.helpers.tap(params.hook, chunk, context),
            placeholder = dust.helpers.tap(params.placeholder, chunk, context),
            maxlength = dust.helpers.tap(params.maxlength, chunk, context),
        ...
        function getValue(key) {
            ...
            if (!key || typeof key !== 'string' || key.replace(/^\s+|\s+$/g, '').length === 0) {
                throw new Error('Invalid @var name');
            }

            // only parse the part after the resource
            // eg. en.GB.blog.my-article:body.excerpt
            keyParts = key.split(':').pop().split('.');

            /* istanbul ignore next */
            if (!keyParts.length) { // DEPRECTED: split will never return empty array, left for deprecation
                return '';
            }
